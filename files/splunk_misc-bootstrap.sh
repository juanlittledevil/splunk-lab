...
splunk_instances="lm cm dep ds mc"
...
do
  ...
  if [ $instance = 'lm' -a -f ${base}/files/license/splunk.license ]; then
    su - splunk -c "export SPLUNK_HOME=${splunk_home}; ${splunk_executable} add licenses ${base}/files/license/Splunk.License -auth admin:changeme"
    su - splunk -c "export SPLUNK_HOME=${splunk_home}; ${splunk_executable} restart"
  fi
done
